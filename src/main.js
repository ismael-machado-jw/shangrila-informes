  ...
  return `
    <div class="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div class="max-w-md w-full bg-white rounded-3xl p-8 border border-slate-200 shadow-xl text-center">
        ...
        ${!CLIENT_ID ? `
          <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-sm text-amber-800 text-left">
            <p class="font-bold mb-1">Falta VITE_GOOGLE_CLIENT_ID</p>
            <p>Configure su Client ID en el panel de Secretos de AI Studio para continuar.</p>
          </div>
        ` : ''}

        <button id="login-btn" class="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold tracking-wide transition-all shadow-lg shadow-indigo-200 flex items-center justify-center gap-3" ${!CLIENT_ID ? 'disabled' : ''}>
